...
import { db, faceapi } from './firebase';
...
const StudentDashboardHome = () => {
    ...
    // ✅ QR Code Scanning (with delay fix)
    useEffect(() => {
        if (step !== 0) return;
        ...
        }, 300); // ✅ Delay of 300ms to let DOM render
    ...
    const handleFaceRecognition = useCallback(async () => {
        ...
        try {
            ...
            if (!detection) throw new Error("No face detected. Try again in good lighting.");
            if (!userData?.faceDescriptor) throw new Error("No face profile found. Contact support.");

            const registeredDescriptor = new Float32Array(userData.faceDescriptor);
            const faceMatcher = new faceapi.FaceMatcher(
                [new faceapi.LabeledFaceDescriptors('user', [registeredDescriptor])]
            );
    ...
    useEffect(() => {
        if (step === 1) handleFaceRecognition();
        ...
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                if (distance <= 200) {
                    ...
                    setError(`Too far from class location (${Math.round(distance)}m).`);
                    setStatus(prev => ({ ...prev, gps: 'failed' }));
                }
                setIsLoading(false);
            },
            (err) => {
                setError("Location access failed. Enable location services.");
                setStatus(prev => ({ ...prev, gps: 'failed' }));
                setIsLoading(false);
            },
    ...
    const handleSubmitAttendance = useCallback(async () => {
        if (status.qr !== 'completed' || status.face !== 'completed' || status.gps !== 'completed') {
            setError("All verification steps must be completed.");
            return;
        }
    ...
    useEffect(() => {
        if (step === 3) handleSubmitAttendance();
