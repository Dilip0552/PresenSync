...
const QrScannerComponent = ({ onScanSuccess, onScanError, setQrScannerReady, qrScanError, setQrScanError }) => {
    ...
    useEffect(() => {
        ...
        const scannerId = "qr-reader"; // Ensure this ID is unique and stable
        ...
        const internalOnScanError = (errorMessage) => {
            setIsCameraStarting(false); // Camera init failed
            onScanError(errorMessage); // Pass error to parent
        };

        html5QrCode.render(internalOnScanSuccess, internalOnScanError)
            .then(() => {
                setIsCameraStarting(false); // Camera has started or attempted to start
                setQrScannerReady(true); // Indicate scanner is ready for use
        ...
        qrCodeScannerRef.current = html5QrCode; // Store the instance
    ...
    return (
        <div id="qr-reader" className="w-full max-w-xs sm:max-w-sm lg:max-w-md aspect-square bg-gray-100 rounded-xl overflow-hidden shadow-inner flex flex-col items-center justify-center relative">
            ...
            {/* Initial content before camera starts/if camera fails */}
